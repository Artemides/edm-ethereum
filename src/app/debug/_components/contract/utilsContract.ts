...
export const getInitalTupleFormState = (abiTupleParam: AbiParameterTuple) => {
  const form: Record<string, any> = {};
  if (abiTupleParam.components.length === 0) return form;

  abiTupleParam.components.forEach((component, componentIndex) => {
    const key = getFunctionInputKey(
      abiTupleParam.name || "tuple",
      component,
      componentIndex
    );
    form[key] = "";
...
export const getInitalTupleArrayFormState = (
